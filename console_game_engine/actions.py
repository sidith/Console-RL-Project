...
funny_lines = [f'They feel bloodlust in their veins.',
...
class MovementAction(DirectionalMovementAction):
    ...
    def perform(self, engine: Engine, entity: Entity) -> None:
        ...
        if not engine.game_map.in_bounds(dest_x, dest_y):
            action_logger.info(
                f"{entity.name}'s destination is out of bounds.")
...
class MeleeAction(DirectionalMovementAction):
    ...
    def perform(self, engine: Engine, entity: Entity) -> None:
        ...
        if blocking_entity:
            ...
            if blocking_entity.name[0] in ['a', 'e', 'i', 'o', 'u', 'A', 'E', 'I', 'O', 'U']:
                action_logger.info(
...
class BumpAction(DirectionalMovementAction):
    def perform(self, engine: Engine, entity: Entity) -> None:
        ...
        if not target:
            action_logger.info(
                f"Nothing interesting is the {entity.name}'s way.")
