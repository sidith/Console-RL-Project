...
class Console:
    ...
    def __init__(
        ...
        if buffer is not None:
            ...
            self._tiles: NDArray[Any] = np.ascontiguousarray(
                buffer, self.DTYPE)
        else:
    ...
    def __deprecate_defaults(
        ...
        if alignment is not None:
            params.append("alignment=%s" %
                          (self.__ALIGNMENT_LOOKUP[alignment],))
    ...
    def rect(
        ...
        lib.TCOD_console_rect(self.console_c, x, y, width,
                              height, clear, bg_blend)
    ...
    def print_frame(
        ...
        _check(lib.TCOD_console_printf_frame(self.console_c,
    ...
    def blit(
        ...
        """Blit from this console onto the  console.
    ...
    def draw_frame(
        ...
        bg_blend: int = tcod.constants.BKGND_SET,
        *,
        decoration: Union[str, Tuple[int, int, int, int,
                                     int, int, int, int, int]] = "┌─┐│ │└─┘",
    ) -> None:
        ...
        if len(decoration_) != 9:
            raise TypeError(
                f"Decoration must have a length of 9 (len(decoration) is {len(decoration_)}.)")
        _check(
